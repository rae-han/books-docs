# Chapter 17: Multi-Paradigm Design (멀티패러다임 설계)

> **참고**: 이 장은 멀티패러다임 노트 6장의 이관본으로, 본문의 `[코드 6-N]` 번호는 원본 기준이다.
...
원서는 이 함수를 `TaskPool` 클래스로 발전시켜(원서 [코드 6-23]~[코드 6-24]) 외부 자원 상황에 따라 `setPoolSize` 메서드로 `poolSize`를 동적으로 변경할 수 있게 확장한다.
...
- **1장**: "함수형과 객체지향은 지향점의 차이"라는 관점이 구조=OOP·로직=FP·흐름 제어=명령형이라는 실전 방법론으로 구체화됐다.
- **5장**: 인터페이스 vs 상속의 선택 기준이 Html·View 클래스 설계에 적용된다.
- **14장**: executeWithLimit·동시성 제어가 TaskRunner/TaskPool이라는 객체지향 확장으로 발전했다.
- **16장**: 이 장의 Html 템플릿 엔진이 다루는 로직 부분은 16장의 리스트 프로세싱 패턴으로 구성된다.
- **18장**: View 클래스가 rune-ts 기반의 프런트엔드 응용으로 확장된다.
