...
- 건전성: soundness, 사운드니스 (정적 타입과 런타임 값의 일치)
...
- 구조적 타이핑: structural typing, 구조적 타입 시스템 (런타임 대응 개념: 덕 타이핑 duck typing)
...
- 리터럴 타입: literal type, 유닛 타입, unit type
- 리팩터링: 리팩토링, refactoring
- 매핑된 타입: mapped type, 맵드 타입
- 메모이제이션: memoization, 메모화
- 명목적 타이핑: nominal typing, 노미널 타이핑
- 목 객체: mock object, 모의 객체, 목
- 봉합: Seam, 이음새, 심
- 불변성: immutability, 이뮤터블
- 브랜드: brand, 브랜딩 (명목적 타이핑용 타입 태그)
- 빅 오 표기법: Big O, 빅오, 점근 표기법
- 사이드카 프록시: sidecar proxy, 사이드카
- 삼각측량: triangulation
- 서비스 메시: service mesh
- 선언 병합: declaration merging, 인터페이스 병합
- 소진 체크: exhaustiveness checking, 철저성 검사, 소진 검사
...
- 잉여 속성 체크: excess property checking, 초과 속성 검사, 엄격한 객체 리터럴 체크
- 자가 테스트 코드: self-testing code
- 전략 패턴: 스트래티지 패턴, Strategy 패턴
- 정보 은닉: information hiding, 정보 은폐
- 제로 트러스트: zero trust, 경계 없는 보안
- 조건부 타입: conditional type, 컨디셔널 타입
- 좁히기: narrowing, 타입 좁히기, 정제, refinement
...
- 타입 가드: type guard, 사용자 정의 타입 가드
- 타입 단언: type assertion, as 단언 ('캐스트'는 부정확한 명칭)
- 태그된 유니온: tagged union, 판별 유니온, discriminated union, 구별된 유니온
- 테스트 주도 개발: TDD
