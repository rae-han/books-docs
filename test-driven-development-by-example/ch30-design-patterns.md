...
test('deposit command', () => {
  ...
  const command: Command = new DepositCommand(account, 50);
  command.execute();
...
test('command queue', () => {
  ...
  const commands: Command[] = [
    new DepositCommand(account, 50),
    new WithdrawCommand(account, 30),
  ];
...
class Dollar {
  ...
  times(multiplier: number): void {
    this.amount *= multiplier; // 원본이 변경됨!
  }
}
...
five.times(3);
expect(five.amount).toBe(15); // 실패! 30이 됨
```
...
class Dollar {
  ...
  times(multiplier: number): Dollar {
    return new Dollar(this.amount * multiplier); // 새 객체 반환
  }
...
expect(five.times(3)).toEqual(new Dollar(15)); // five는 여전히 5
```
...
class NullUser implements User {
  getName(): string { return "Anonymous"; }
...
// 1단계: null 체크가 있는 테스트
test('no discount', () => {
  const order = new Order(100, null); // 할인 없음
...
// 2단계: Null Object 도입 후
test('no discount', () => {
  const order = new Order(100, Discount.none());
  expect(order.total()).toBe(100);
});
...
class TestCase {
  ...
  run(result: TestResult): TestResult {
    result.testStarted();
    this.setUp();        // 하위 클래스가 오버라이드
    try {
      (this as any)[this.name](); // 하위 클래스가 정의한 테스트 메서드
    } catch {
      result.testFailed();
    }
    this.tearDown();     // 하위 클래스가 오버라이드
    return result;
  }
...
// 리팩토링 전: 두 클래스에 비슷한 코드
class CsvExporter {
  export(records: Record[]): string {
    const header = "name,email,age\n";         // 다름
    let body = "";
    for (const r of records) {
      body += this.formatCsv(r);               // 다름
    }
    return header + body;
  }

  private formatCsv(r: Record): string { /* ... */ return ""; }
}
...
class JsonExporter {
  export(records: Record[]): string {
    ...
    let body = "";
    for (const r of records) {
      body += this.formatJson(r);              // 다름
    }
    return header + body + "]";
  }

  private formatJson(r: Record): string { /* ... */ return ""; }
}
...
abstract class Exporter {
  export(records: Record[]): string {
    let result = "";
    result += this.header();                    // 추상
    for (const r of records) {
      result += this.formatRecord(r);           // 추상
    }
    result += this.footer();                    // 추상
    return result;
  }

  protected abstract header(): string;
  protected abstract formatRecord(r: Record): string;
...
class TaxCalculator {
  constructor(private policy: TaxPolicy) {}

  calculate(order: Order): Money {
    return this.policy.calculate(order.total());
  }
}
```
...
```typescript
test('US tax', () => {
  const policy: TaxPolicy = new USTaxPolicy();
  expect(policy.calculate(Money.dollar(100))).toEqual(Money.dollar(8));
});

test('UK tax', () => {
  const policy: TaxPolicy = new UKTaxPolicy();
  expect(policy.calculate(Money.dollar(100))).toEqual(Money.dollar(20));
});

test('no tax', () => {
  const policy: TaxPolicy = new NoTaxPolicy();
  expect(policy.calculate(Money.dollar(100))).toEqual(Money.zero());
});
```
...
class TestCase {
  ...
  constructor(name: string) {
    this.name = name; // 메서드 이름을 문자열로 저장
  }
  ...
  run(result: TestResult): TestResult {
    ...
    this.setUp();
    try {
      const method = (this as any)[this.name]; // 리플렉션으로 메서드 조회
      method.call(this);                        // 동적 호출
    } catch {
      result.testFailed();
    }
    this.tearDown();
...
class RealExchangeRateService implements ExchangeRateService {
  getRate(from: string, to: string): number {
    // HTTP API 호출 (느림, 외부 의존)
    return this.httpClient.get(`/rates/${from}/${to}`);
  }
}
...
test('currency conversion', () => {
  const rates = new StubExchangeRateService();
  rates.setRate("CHF", "USD", 1.5);
  const bank = new Bank(rates);
  expect(bank.convert(Money.franc(15), "USD")).toEqual(Money.dollar(10));
});
```
...
```typescript
// 개별 테스트
const test1 = new MoneyTest("testMultiplication");

// 테스트 그룹
const moneySuite = new TestSuite();
moneySuite.add(new MoneyTest("testMultiplication"));
moneySuite.add(new MoneyTest("testEquality"));

// 전체 스위트 (스위트 안에 스위트)
const allTests = new TestSuite();
allTests.add(moneySuite);              // 스위트 추가
allTests.add(new BankTest("testRate")); // 개별 테스트 추가
...
```typescript
const result = new TestResult(); // 빈 수집 매개변수
...
console.log(result.summary()); // "3 run, 1 failed"
```
...
```typescript
// 반환값 방식 (번거로움)
const result1 = test1.run();
const result2 = test2.run();
const result3 = test3.run();
