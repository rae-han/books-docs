...
> 빨강 — 실패하는 작은 테스트를 작성한다. 처음에는 컴파일조차 되지 않을 수 있다. 초록 — 빨리 테스트가 통과하게끔 만든다. 이를 위해 어떤 죄악을 저질러도 좋다. 리팩터링 — 일단 테스트를 통과하게만 하는 와중에 생겨난 모든 중복을 제거한다.<br>— 켄트 벡 (위치: 서문)

> 두려움이 지루함으로 바뀔 때까지 테스트를 작성하라.<br>— 켄트 벡 (위치: 서문)

> 동시에 여러 개의 공을 공중에 띄워놓고 저글링할 때처럼, 프로그래밍할 때도 잠시만 집중을 놓치면 모든 게 무너져 버릴 것 같은 느낌을 받은 적이 많다. 테스트 주도 개발은 이런 느낌을 감소시키는 데 도움이 되고, 그 결과로 신속한 느긋함을 얻게 된다. 테스트 주도 개발 스타일로 작업하면 한 번에 딱 하나의 공만 공중에 띄우는 느낌이 들고, 그렇기 때문에 그 공에 충분히 집중할 수 있다.<br>— 마틴 파울러 (위치: 한국어판 후기)
