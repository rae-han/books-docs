...
> 어떤 새가 오리처럼 걷고, 오리처럼 헤엄치며, 오리처럼 꽥꽥 소리를 낸다면 나는 이 새를 오리라고 부를 것이다.<br>- 제임스 윗콤 릴리(James Whitcomb Riley), 덕 테스트 (위치: 부록 B)
...
- **단위**: 챕터당 1파일 (`summary/chNN-*.md`, 상세 노트와 동일 파일명. 부록 포함)
- **구성**: 핵심 질문, `## 핵심` 불릿, 코드 한 컷(해당 시), 닫는 핵심 통찰. 상세 규칙은 루트 CLAUDE.md "요약 노트 작성 규칙" 참조
- **상태**: Ch1~15 + 부록 A/B/C 전체 작성 완료 (2026-08-09)
