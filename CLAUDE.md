...
### 24. effective-typescript-2nd/ - Effective TypeScript, 2nd Edition (댄 밴더캄, O'Reilly, 2024)
- 10개 챕터·83개 아이템 + README. **영어 원서 2판** 기반(1판 한국어판: "이펙티브 타입스크립트" 인사이트 2021). "타입 = 값의 집합"(Item 7)을 렌즈로 초중급 사용자를 전문가로 끌어올리는 Effective 시리즈 실전 지침서
- **파일 단위가 저장소 표준의 예외**: 아이템이 사실상 챕터 역할이라 노트를 `notes/item{NN}-{영문-kebab-case}.md` 83개로 분리(`ch{NN}-*` 아님). 챕터 개요는 README 목차의 챕터 그룹(`### Chapter N: 제목 (Item a~b)`) 서두 문단에
- **노트 구성**: `# Item N: 한글 제목 (English Title)` → `## 핵심 질문` → 번호 섹션(`## 1.` …) → `## 기억해야 할 것들`(원서 시그니처 "Things to Remember" - 별도 `## 요약` 없음). `## 다른 챕터와의 관계` 생략 확정(본문 속 `Item N` 상호 참조가 촘촘)
- **시그니처**: twoslash 코드 표기(`// ~~~ 에러 메시지`(컴파일러 출력이라 영어 원문 유지)·`// ^? 타입`), 콜아웃 3종(`> **핵심 통찰**:`·`> **실무 팁**:`·`> **참고**:`), **단순 포함/집합 관계는 ASCII 그림 대신 ⊂ 표기·불릿**(사용자 피드백 - 그림은 구조가 그림 없이는 전달되지 않을 때만)
- **코드**: 원서가 이미 TS라 병기 없음. ⚠️ OCR 코드 블록이 줄 단위로 흐트러져 있어(객체 리터럴 행 분리 등) 전량 재구성하며 작성
- origin 분리 미완: 0~10, 11=부록(1판↔2판 아이템 매핑, 15581행~), 99=색인(15688행~) 예정. 장 마커는 `Chapter N.`·`Item N:` 패턴으로 안정적
- **Notion 업로드 완료**: Must reads → "이펙티브 타입스크립트 2판" 페이지의 인라인 DB "아이템"(83행). 파트가 없는 책이라 `Part` 속성 값에 `Ch1`~`Ch10`을 넣고 `Chapter`는 아이템 번호(1~83). 상세는 책 README `## Notion DB 구조`
- 상태: **완료** (Item 1~83 노트 + README + QUOTES 11건·KEYWORDS 16항목 + Notion 업로드)
...
├── notes/         ← 상세 챕터 노트 (chNN-*.md + tips.md·hands-on 등 책 고유 노트 자산)
...
**요약 노트는 상세 노트에 종속된 압축본이 아니라, 챕터의 핵심 내용을 자체적으로 담은 독립 학습 노트다.** 상세 노트 없이 이 노트만 읽어도 그 챕터를 학습/복습할 수 있어야 한다. 2026-08-09 포맷 확정, 같은 날 독립 학습 노트 관점으로 개정 (개정 샘플: head-first Ch01, pragmatic Ch1, clean-code Ch1, js-react-pattern Ch01).

**단위 결정 규칙** (책마다 하나의 단위만, 혼합 금지. 택한 단위는 그 책 README에 기록)

1. 기본은 **챕터당 1파일** `summary/chNN-*.md` (상세 노트와 동일 파일명 미러)
2. 챕터 내용이 얇은 책은 **파트당 1파일** `summary/partN-*.md`, 파트도 없으면 **책당 1파일** `summary/summary.md`

**챕터 요약 포맷** (대략 40~90줄, 상세 노트의 1/3 이하 분량 감각)

1. 제목: `# <상세 노트 제목과 동일> - 요약`. **상세 노트 링크는 넣지 않는다**
2. `**핵심 질문**: 한 줄` (헤더 없이 인라인)
3. 본문: **주제별 소제목(`##`) 섹션 3~7개** - 상세 노트의 큰 흐름을 따르되 재구성 가능. 산문과 불릿을 섞어 개념 정의, 왜 그런지, 대표 예시까지 담는다. `## 핵심` 같은 단일 불릿 덤프 섹션은 쓰지 않는다
4. **핵심 코드**: 개념이 코드로 표현되는 책/챕터(디자인 패턴, 리팩터링 기법, 구현 중심 장)는 핵심 코드 필수 - 패턴 구조나 before/after를 이해에 필요한 최소 분량으로. 개념 중심 챕터는 생략 가능
5. 책 시그니처 요소 유지: Red Flags, 디자인 원칙(누적), 패턴 정의 박스, `## 이 장의 Tip 목록` 등은 요약에서도 형식 그대로 유지
6. 닫는 `> **핵심 통찰**:` 한 문장

**덜어내는 것**: 예제의 장황한 전개 서사, 다단계 리팩터링의 중간 단계, Java 병기(`<details>` - 요약에는 TS만), 큰 클래스 다이어그램, Q&A류, 연습 문제, `## 다른 챕터와의 관계`, 원서 복습 요소

**진행 방식**: 책마다 구조가 달라 요약 기준이 다르므로, **각 책의 첫 요약 1개를 샘플로 보여주고 사용자 확인 후** 그 책 전체를 진행한다. 한 번에 최대 4권씩 배치로 진행 가능. 문장부호는 공통 규칙(대시는 하이픈, 가운뎃점 제한)을 따른다.
...
- **폴더명**: 원서 제목 kebab-case. 한국어판 제목이 크게 달라 검색이 어려우면 한글 병기 가능(`the-essence-of-object-orientation-객체지향의-사실과-오해`)
- **목차 원본이 아직 없으면** README 목차 표를 원서(O'Reilly 등) 목차로 채우되 **잠정치임을 명시**하고, 한국어판 실제 장 제목으로 교정하는 항목을 `## 작성 상태`에 남긴다

## README 표준 구조

모든 책 폴더의 README.md는 아래 뼈대를 따른다. 4·5·6번의 **목차 표는 Notion 챕터 DB의 소스**이므로 형식을 지킬 것.
...
4. `## 목차` ★: 파트가 있으면 `### Part N: 제목 (Ch a-b)` 그룹 아래, 없으면 단일(또는 책별 분류 딱지 기준 그룹) 표
   ...
   - **핵심 단어**: 챕터당 3~6개, **쉼표 구분** (2026-08-09에 `·` 구분에서 전환). 나중에 "그 개념 어느 장이더라?" 검색용. **같은 개념은 항상 같은 표기**로 쓰며, 대표 표기·동의어는 루트 `KEYWORDS.md`(용어 사전)에서 확인·등록한다 - 새 핵심 단어를 쓰기 전에 사전에 이미 있는 표기인지 확인하고, 동의어가 새로 생기면 사전에 추가
   - **한 줄 요약**: ~80자 이내
...
- **가운뎃점(·) 사용 제한**: 구분자로 쓰지 않는다. 목차 표 핵심 단어 등 나열 구분은 **쉼표**(공백 있는 ` · ` 1,442건 치환 완료, 코드 펜스 안 다이어그램은 보존), 복합 개념 결합은 **슬래시**(`설계/구현 단계`), 본문 명사구 나열은 **쉼표 또는 '와/과'**(`나는 행동과 꽥꽥거리는 행동`). 예외: **인명 병기**(`리처즈·포드`)는 허용. 기존 본문에 남은 결합용 `·`(약 9,200건)는 발견 시 점진 수정한다. 이유: 키보드로 입력 가능한 문자 우선 + AI 생성 문체 흔적 제거 (사용자 지시)
- **책/작품 제목은 큰따옴표**: 낫표(`『』`·`「」`)를 쓰지 않고 큰따옴표로 표기한다 - `"클린 코드"`, `"Simple Made Easy"`. 2026-08-09에 기존 490건 전량 치환 완료(코드 펜스 안은 보존). ⚠️ **꺾쇠(`<제목>`)는 금지** - 마크다운이 raw HTML 태그로 파싱해 영문 제목이 렌더링에서 사라진다(`<Design Patterns>`). 원서 제목을 이탤릭으로 강조하던 기존 표기(`원서 *Mastering API Architecture*`)는 그대로 둔다
- **화살표(→) 지양**: 신규 산문에서는 화살표 특수문자 대신 서술로 풀거나 `->`를 쓴다. 단 기존 문서와 ASCII 다이어그램의 `→`는 유지(다이어그램 정렬 훼손 위험, 수천 건)
...
- **한글은 리터럴 그대로 (유니코드 이스케이프 절대 금지)**: `notion-create-pages`/`notion-update-page`의 `content`에 한국어를 넣을 때 `\uXXXX` 유니코드 이스케이프로 손수 변환하지 말고 **한글 문자를 그대로** 넣는다. 수동 이스케이프 변환은 음절 손상("캡슐화"→"캐슸화", "말썽꾼"→"말썰꾼" 등)을 일으켜 원본과 다른 내용이 올라간다(head-first-design-patterns 15개 챕터 전량 재업로드 사례, a-common-sense-guide 20개 챕터 전량 재업로드 사례 - 돈다→돌다·잰다→잴다·옮겨야→옆겨야·썼다면→쎜다면·뺀→뻐·붓는다→부는다·숟가락→숨가락 등 챕터당 3~10곳 손상. mastering-api-architecture 11개 챕터 전량 재업로드 사례(2026-08-09) - 바꾼다→바꿈다·끄집어내는→끔지버내는·악몽→악몣·끊고→끕고·곧바로→곷바로·뼈대→뾼대. `update_content` 개별 치환으로 고치려 해도 치환 문자열 자체가 또 깨지므로, **본문 전체를 리터럴 한글로 `replace_content`** 하는 것이 유일하게 확실한 해법). 또한 `__unparsedToolInput`으로 raw JSON을 조립하지 말고 정상 파라미터(`parent`/`pages` 등)로 전달한다 - JSON 인코딩은 harness가 처리하며, raw JSON은 `InputValidationError`/hex escape 파싱 에러를 반복 유발한다. **업로드 후 반드시 1~2개 페이지를 `notion-fetch`로 열어 한글 손상 여부를 눈으로 확인**하고, 손상 발견 시 `replace_content`로 한글 리터럴 재업로드한다.
  - ⚠️ **속성(`update_properties`) 값에도 똑같이 적용된다**(2026-08-10 effective-typescript-2nd 사례). 본문만 신경 쓰다 `핵심 단어`·`핵심 요약`에서 8건 손상 - 바꾼다→바꿈다, 얕은→얇은, 불쾌한→불쿾한, 켠다→켜다, 뜬다→뜼다, 끊어라→끕어라, 가늠하라→가늘하라, 옮겨라→옆겨라
  - **손으로 이스케이프를 계산하지 말 것**: 고치려다 또 틀리는 일이 반복된다(위 사례에서 수정 시도 4건이 재차 손상). 이스케이프가 불가피하면 `python3 -c "print('문자열'.encode('unicode_escape').decode())"`로 **정확한 시퀀스를 생성해서 복사**한다
  - **검증은 뷰 필터로 싸게**: 전량 조회는 응답이 매우 크다. `update-view`로 `FILTER "Chapter" = N OR ...`를 임시로 걸어 해당 행만 확인하고 `CLEAR FILTER`로 원복한다
...
PDF에서 추출한 `origin.md` 원본 파일을 챕터별로 분리하는 작업. 내용 수정 없이 라인 기반으로 분할하며, 분리 파일은 책 폴더의 **`origin/` 하위**에 둔다 (`origin.md` 통짜 원본은 책 폴더 루트에 그대로).
...
한 책의 챕터 노트를 다 쓰면 **따로 묻지 말고 아래를 이어서 처리**한다. 각 항목의 상세 규칙은 위의 해당 절을 따른다.
