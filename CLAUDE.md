...
상세 노트에서 핵심만 추린 복습용 노트. 2026-08-09 포맷 확정 (샘플: philosophy Ch4, objects Ch1).

**단위 결정 규칙** (책마다 하나의 단위만, 혼합 금지. 택한 단위는 그 책 README에 기록)

1. 기본은 **챕터당 1파일** `summary/chNN-*.md` (상세 노트와 동일 파일명 미러): 챕터 요약이 평균 25줄 이상 나오는 밀도의 책
2. 챕터 요약이 그보다 얇으면 **파트당 1파일** `summary/partN-*.md`: 파일당 80~300줄 확인
3. 파트가 없거나 파트로 묶어도 얇으면 **책당 1파일** `summary/summary.md`
4. 역방향 가드: 챕터 요약이 40줄을 넘으면 단위 문제가 아니라 덜 덜어낸 것. 요약 기준을 재점검

**챕터 요약 포맷** (15~40줄)

1. 제목: `# <상세 노트 제목과 동일> - 요약`
2. `> 상세 노트:` 인용 한 줄에 상세 노트 링크(경로는 `../notes/chNN-*.md`)
3. `**핵심 질문**: 한 줄` (헤더 없이 인라인)
4. `## 핵심`: 불릿 6~8개, `- **개념**: 한두 문장` 형태로 개념 정의와 주장을 한 불릿에 담는다 (상세 노트의 `## 요약` 복사가 아니라 노트 전체에서 다시 추린다. 책 없이 읽혀야 하므로 개념 정의 포함)
5. `## 코드 한 컷` (선택): 챕터의 개념이 코드 그 자체일 때만, before/after 최소 대비 한 쌍
6. 책 시그니처 요소 압축 (선택): Red Flags, 설계 원칙, Tip 등을 1줄씩
7. 닫는 `> **핵심 통찰**:` 한 문장

**덜어내는 것**: 예제 전개 서사, 단계별 리팩터링 과정, Java 병기(`<details>`), 클래스 다이어그램, Q&A류, 연습 문제, `## 다른 챕터와의 관계`
