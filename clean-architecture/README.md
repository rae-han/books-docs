...
| 부록 | 제목 | 핵심 단어 | 한 줄 요약 |
|----|------|-----------|-----------|
| A | [Architecture Archaeology](notes/appendix-architecture-archaeology.md) | 아키텍처 고고학 | Uncle Bob 45년 프로젝트 회고 — 원칙이 도출된 경험들 |
...
> 빨리 가는 유일한 방법은 제대로 가는 것이다(The only way to go fast, is to go well).<br>— 로버트 C. 마틴 (위치: Ch1)

> 좋은 아키텍트는 내리지 않은 결정의 수를 최대화한다.<br>— 로버트 C. 마틴 (위치: Ch15)

> 프레임워크는 도구일 뿐, 삶의 방식이 아니다.<br>— 로버트 C. 마틴 (위치: Ch32)
