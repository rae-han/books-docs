...
이 챕터는 Docker의 핵심 개념을 알고 있다는 전제로 진행한다. [Docker 완전 가이드](../README.md)에서 다룬 내용이 AWS 컨테이너 서비스와 어떻게 대응되는지 먼저 파악해두면 이해가 빠르다.

| Docker 개념 | AWS 대응 | Docker 가이드 참고 |
|-------------|----------|-------------------|
| Docker Hub / Registry | **ECR** | [Ch 02 이미지](../../docker-complete-guide/notes/ch02-images.md) |
| `docker run` | **ECS Task** | [Ch 03 컨테이너](../../docker-complete-guide/notes/ch03-containers.md) |
| Dockerfile | ECR에 푸시할 이미지 빌드 | [Ch 04 Dockerfile](../../docker-complete-guide/notes/ch04-dockerfile-deep-dive.md) |
| `docker-compose.yml` | **ECS Task Definition + Service** | [Ch 07 Compose](../../docker-complete-guide/notes/ch07-docker-compose.md) |
| `docker compose up --scale` | **ECS Service Auto Scaling** | [Ch 08 Compose 고급](../../docker-complete-guide/notes/ch08-docker-compose-advanced-patterns.md) |
| 멀티 스테이지 빌드, alpine | Fargate 콜드 스타트 최적화 | [Ch 09 이미지 최적화](../../docker-complete-guide/notes/ch09-image-optimization.md) |
...
Docker 가이드 [Ch 10 보안](../../docker-complete-guide/notes/ch10-security.md)에서 다룬 `docker scout` 같은 로컬 스캔 도구와 함께 사용하면, 로컬에서 먼저 스캔하고 ECR에서 한 번 더 검증하는 이중 방어가 가능하다.
...
Next.js SSR 서버를 프로덕션 수준으로 ECS Fargate에 배포하는 전체 아키텍처를 구성한다. Docker 가이드 [Ch 12 Node.js Docker Best Practices](../../docker-complete-guide/notes/ch12-nodejs-docker-best-practices.md)에서 다룬 최적화 기법을 기반으로 한다.
...
> **핵심 통찰**: Docker Compose에서 ECS로 전환할 때 가장 큰 마인드셋 전환은 **"컨테이너는 언제든 사라질 수 있다"**는 것이다. 상태(*state*)를 컨테이너 외부(S3, RDS, ElastiCache)에 저장하는 것이 클라우드 네이티브 설계의 핵심이다. 이 원칙은 Docker 가이드 [Ch 05 볼륨과 바인드 마운트](../../docker-complete-guide/notes/ch05-volumes-and-bind-mounts.md)에서 강조한 "데이터를 컨테이너 밖에 저장하라"의 연장이다.
...
> **실무 팁**: 이미 Kubernetes를 사용 중이거나 멀티 클라우드 전략이 있다면 EKS를, 그 외 대부분의 경우에는 ECS + Fargate를 선택하라. Docker 가이드 [Ch 15 Docker 너머](../../docker-complete-guide/notes/ch15-beyond-docker.md)에서 설명한 K8s 핵심 개념(Pod, Deployment, Service, Ingress)을 EKS에서 그대로 사용한다.
