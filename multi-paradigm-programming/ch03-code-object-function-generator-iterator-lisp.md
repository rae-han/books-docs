...
`find even [1, 3, 5]`의 결과는 `Maybe` 타입으로, 현재 리스트에 짝수가 없기 때문에 `Nothing`을 반환한다. `fromMaybe` 함수는 `Nothing`일 경우 기본값으로 전달한 0을 반환한다.
...
다시 타입스크립트로 돌아와 이터러블 프로토콜을 따르는 `find` 함수를 만들겠다.
...
이제 `find` 함수를 함수형 방식으로 구현해 보겠다. 지연 평가를 지원하는 `filter`를 사용하여 n `ext()`를 한 번만 평가한다면 이는 `find`와 동일한 로직과 효율을 가지게 된다. 한 번만 평가한다면 `yield`는 사실상 `return`과 같기 때문이다.
...
위의 세 가지 방식 모두 명령형 코드로 작성한 `find` 함수와 동일한 효율을 제공하면서도 코드가 간결하다. 각 패러다임으로 작성한 코드는 서로를 완전히 대체할 수 있으며, 필요하다면 섞어서 사용할 수도 있다.
...
①번 방식(옵셔널 체이닝)은 값이 실제로 없을 때도 런타임 에러 없이 `undefined` 처리된다. 반면 ②번 방식(Non-null 단언)은 값이 없는데도 존재한다고 단언하므로 실제로 값이 `null`이나 `undefined`라면 런타임 에러가 발생할 수 있다.

②번 방식은 개발자가 '이 로직에서는 값이 반드시 존재하도록 설계했다'는 의도를 언어에 전달하는 수단이다. 즉 '이곳에서는 `null`이나 `undefined`가 나타나지 않는 상황이며, 만약 실제로 값이 없다면 그것은 설계가 어긋난 상황이므로 런타임 에러가 발생해야만 한다'는 의도를 표현하며 구현한 것이다. 따라서 에러가 발생한다면 개발자는 `!`를 없애는 것이 아니라 런타임에서 값을 찾지 못하는 이유를 찾아 해결해야 한다.

### 4.5 every 함수

`every` 함수는 주어진 함수 `f`가 모든 요소에 대해 `true`를 반환하면 최종 결과로 `true`를, 그렇지 않으면 `false`를 반환해야 한다.
...
함수형 프로그래밍에서는 리스트를 단계별로 변환하며 최종 결과를 도출하는 방식으로 사고한다. 먼저 리스트의 모든 요소를 `boolean` 값으로 변환한 뒤 이들을 `&&` 연산자로 모두 연결하면 원하는 결과를 쉽게 얻을 수 있다.
...
`reduce`의 누적 함수로 `a && b`를 전달하여 `(boolean && boolean && boolean)`과 동일한 효과를 냈다. 참고로 "한 번의 `reduce`에서 `f(b)`와 같이 구현하지 않고 왜 `map`과 `reduce`로 나누어 순회하는 거지?"라는 의문이 생길 수도 있다. 하지만 지연 이터레이터의 특성상 각 요소가 `map`을 통과한 직후 즉시 `reduce`에 소비되므로 실제로는 한 번만 순회하면서 O(n) 시간이 걸린다.

### 4.6 some 함수

`some` 함수도 비슷한 방식으로 구현할 수 있다. 모든 값을 `boolean`으로 만든 후 `||` 연산자로 묶으면 된다.
...
`some`에서는 `.filter(a => a).take(1)`을 통해 `true`를 하나라도 만나면 더 이상 순회하지 않는다. `every`에서는 `.filter(a => !a).take(1)`을 통해 `false`를 하나라도 만나면 순회를 종료한다. 이로써 반복문을 `if () break;`로 종료한 것처럼 효율이 좋아졌다.
...
`accumulateWith` 함수를 사용하여 `every`와 `some` 함수의 공통 로직을 추상화했다. 서로 다른 로직을 인자로 전달하는데, 특히 논리가 담긴 코드를 함수로 전달하여 조립하는 면이 특별하다. 이처럼 함수형 프로그래밍은 리팩터링하기도 좋으며 유지보수성이 뛰어나다.
...
`concat` 함수는 여러 이터러블을 인자로 받아 각 이터러블의 요소를 순차적으로 생성한다. 이때 배열 전체를 한 번에 결합하는 대신 필요한 요소를 하나씩 순회하며 생성한다.

제너레이터 `concat`은 원본 배열을 변경하지 않고 필요한 시점에만 요소를 생성하므로, 프로그램에서 원본 배열을 여러 번 사용해야 하는 상황에 특히 유용하다. 또한 `take`나 `some`과 함께 사용하면 필요한 만큼만 요소를 처리할 수 있어 효율적이다.
