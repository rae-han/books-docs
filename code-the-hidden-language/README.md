...
> 코드는 본질적으로 **번역(translation)**이다. 영어를 프랑스어로 번역하듯, 우리는 문자를 빛의 깜빡임으로 "번역"할 수 있다. 중요한 것은 보내는 사람과 받는 사람이 **같은 규칙**을 공유하는 것이다.<br>— 찰스 페촐트 (위치: Ch1)

> 컴퓨터가 작동하는 근본 원리는 두 아이가 손전등으로 대화하는 것과 본질적으로 같다. 둘 다 "두 가지 상태"의 조합으로 복잡한 정보를 전달한다. 차이점은 오직 **속도**와 **규모**뿐이다.<br>— 찰스 페촐트 (위치: Ch1)

> 양각 문자가 실패한 이유는 "눈을 위해 설계된 코드를 손끝에 강제한 것"이기 때문이다. 매체(medium)에 맞는 코드를 설계해야 한다.<br>— 찰스 페촐트 (위치: Ch3)

> 점자는 1800년대에 만들어진 "6비트 컴퓨터 코드"라고 할 수 있다. 고정 길이, 이진 상태, 이스케이프 코드까지 — 현대 컴퓨터의 문자 인코딩과 놀라울 정도로 동일한 구조를 가지고 있다.<br>— 찰스 페촐트 (위치: Ch3)

> 10진법은 "우주적 진리"가 아니라 "생물학적 우연"이다. 외계인이 손가락 7개라면 7진법을 쓸 것이고, 그들의 수학도 완벽하게 작동할 것이다.<br>— 찰스 페촐트 (위치: Ch9)

> 비트는 정보의 "원자"다. 물질의 원자가 더 이상 쪼갤 수 없는 기본 단위이듯, 비트는 더 이상 단순화할 수 없는 정보의 기본 단위다.<br>— 찰스 페촐트 (위치: Ch11)

> 릴레이, 진공관, 트랜지스터는 모두 "전기로 제어하는 스위치"라는 같은 역할을 한다. **논리 게이트의 기능은 구현 기술이 바뀌어도 동일하다.**<br>— 찰스 페촐트 (위치: Ch15)

> RAM은 호텔과 같다. 어떤 방이든 번호만 알면 바로 갈 수 있다. 반면 카세트 테이프는 고속도로 — 원하는 곡을 들으려면 앞의 곡들을 빨리 감기해야 한다.<br>— 찰스 페촐트 (위치: Ch19)

> 제어 장치는 오케스트라의 지휘자다. 악기(ALU, 레지스터, 메모리)는 모두 준비되어 있지만, 지휘자가 "지금 바이올린 연주!", "이제 드럼!"이라고 지시해야 음악(계산)이 된다.<br>— 찰스 페촐트 (위치: Ch23)

> 폴링은 5분마다 현관 문을 열어 택배가 왔나 확인하는 것이고, 인터럽트는 초인종이 울릴 때만 문을 여는 것이다.<br>— 찰스 페촐트 (위치: Ch25)

> 손전등의 ON/OFF에서 시작하여 월드 와이드 웹에 도달했다. 이 모든 것의 근본은 동일하다 — **두 가지 상태(0과 1)의 조합**. Chapter 1의 손전등이 깜빡이는 것과 본질적으로 같다 — 차이는 오직 속도와 규모뿐이다.<br>— 찰스 페촐트 (위치: Ch28)
